...
def get_vscode_dir() -> pathlib.Path:
    match uname := platform.system():
        ...
        case "Linux":
            ...
            settings_base = XDG_CONFIG_HOME
        case _:
            abort("Unsupported system %s", uname)

    return (settings_base / "Code" / "User").expanduser()
...
def verify_shared_settings(user: pathlib.Path) -> None:
    logger.info("Veryfing shared settings ...")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception("Unhandled error. This is abug.")
        sys.exit(2)
